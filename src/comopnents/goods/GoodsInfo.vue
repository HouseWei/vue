      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          ...
          <p>
            ...
            <!-- 1. 经过分析发现,按钮 属于goodsinfo页面,数字 属于 numberbox组件 -->
...
export default {
  ...
  methods: {
    ...
    enter(el, done) {
      ...
      //小球动画优化思路;
      //1. 先分析导致 动画 不准确的 本质原因:我们把 小球 最终 位移到到的位置,已经局限在了某一分辨率下的 滚动条为滚动的情况下
      //2. 只要分辨率 和 测试的时候不一样,或者 滚动条有一定的滚动距离之后,问题就会出现
      //3. 因此,我们经过分析,得到结论: 不能把 位置的 横纵坐标写死了, 而是应该 根据不同情况, 动态计算 这个坐标值
      //4. 经过分析,得出解题思路: 让先得到 徽标的 横纵坐标,再得到 小球的 横纵坐标,然后让 y值求差,x值求差,得到的结果,就是要移动的距离
